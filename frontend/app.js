// Tabs
    ...
    async function startAsyncRun(job_title, payload){
      ...
      let ack = {};
      try { 
        ack = JSON.parse(ackText.trim().replace(/^=\s*/, "")); 
      } catch {}
      ...
      ack.job_id = jobId; // für Folge-Schritte verfügbar machen
      ...
      if (final?.status === "done"){
        ...
        jobLine.textContent = "Noch in Arbeit …";
      }
    }
    ...
    if (speakerForm) {
      ...
      // Auto-Refresh wenn Tab geöffnet wird
    ...
    async function checkHardware(statusEl, selectEl){
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){
        statusEl.textContent = "❌ Kein getUserMedia verfügbar (Browser zu alt?).";
        return false;
      }
      if (!isSecure()){
        statusEl.textContent = "❌ Unsichere Seite. Mikrofon benötigt HTTPS oder localhost.";
        return false;
      }
    ...
    async function startRecording(selectEl, statusEl, meterEl, timerEl, fileInput, label){
      try{
        ...
        statusEl.textContent = `❌ Zugriff verweigert: ${err.message||err}`;
        return false;
      }
      mic.chunks = [];
